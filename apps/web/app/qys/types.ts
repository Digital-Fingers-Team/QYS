...
export type Challenge = { id: number; title: string; description: string; image?: string | null; reward: number; status: string; category: string; participants: number; deadline: string; joined?: boolean; _count?: { participations: number } };
...
export type Complaint = { id: number; userId: number; centerId?: number | null; centerReviewStatus?: string | null; showProgress?: boolean; resolvedAt?: string | null; title: string; description: string; type: string; status: string; createdAt: string; user?: { name: string }; center?: { name: string; location: string } | null };
